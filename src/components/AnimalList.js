...
{nizZivotinja.map((zivotinja) => (
          <tr className='zivotinjeTable' key={zivotinja.id}> <span>VRSTA: {zivotinja.vrsta} </span> <span>IME: {zivotinja.ime} </span> <span>DATUM RODJENJA: {zivotinja.datumRodjenja ? zivotinja.datumRodjenja : "Datum rodjenja nepoznat"} </span> </tr>
        ))} 
</tbody>
        </table>
    </div>
